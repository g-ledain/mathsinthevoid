...
Let <span>$\mathcal{C},\mathcal{D}$</span> be braided monoidal categories. A lax monoidal functor <span>$F: \mathcal{C}\to\mathcal{D}$</span> is said to be **braided** if the following diagram commutes:
TO DO: DIAGRAM

A colax such functor <span>$F$</span> is said to be braided if the following diagram commutes:
TO DO: DIAGRAM
...
When <span>$F,G$</span> are colax monoidal, a colax monoidal natural transformation <span>$\alpha: F\Rightarrow G$</span> is said to be braided if CONDITION

When <span>$F,G$</span> are bilax, a natural transformation <span>$F\ Rightarrow G$</span> is said to be bilax monoidal if it is both lax monoidal and colax monoidal.
