...
Let $S$ be a set. The free monoidal category $\text{Free}(S)$ on $S$ can be described recursively as:
 - Objects: 
    - An object $1$ not in $S$ 
    - An object $s$ for each $s\in S$ 
    - For each $s\in S$ and each $x \in \mathcal{C}$, an object $s\otimes x$ and an object $x\otimes s$ 
 - Morphisms:
   - For each $x \in \mathcal{C}$, a map $id_x: x\to x$
   - For each $x\in\mathcal{C}$, maps $\rho_x: x\otimes 1 \to x$ and $\tilde{\rho_x}: x\to x\otimes 1$
   - For each $x\in\mathcal{C}$, maps $\lambda_x: 1\otimes x \to x$ and $\tilde{\lambda_x}: 1\otimes x \to x$
   - For each $x,y,z\in \mathcal{C}$, maps $\alpha_{x,y,z}: x\otimes (y\otimes z) \to (x\otimes y)\otimes z$ and $\tilde{\alpha_{x,y,z}}: (x\otimes y) \otimes z$

Let's see how we can use the above theorem to conclude that a certain diagram commutes: Consider $\rho_x \circ \tilde{\rho_x}: x\to x$. By the description above, there is a unique map $x\to x$, which is $id_x: x\to x$. Hence, $\rho_x \circ \tilde{\rho_x} = id_x$. \
More generally, note that there is at most one morphism between two objects in $\text{Free}(S)$, so every diagra, in $\text{Free}(S)$ commutes.

*Corollary: "Every diagram commutes"*\
Let $\mathcal{C}$ be a monoidal category. Then any diagram in $\mathcal{C}$ composed entirely of unitors and associators commutes which exists entirely for formal reasons commutes.

This corollary is a bit mealy-mouthed in that it has that irritating caveat "which exists entirely for formal reasons". The point here is that a monoidal category $\mathcal{C}$ can contain objects $x,y,z$ such that $x\otimes(y\otimes z) = (x\otimes y)\otimes z$. A more careful statement of the corollary actually amounts to a proof: specifically, let $S$ be some collection of objects in a monoidal category $\mathcal{C}$. Then by the universal property, there exists a unique (up to isomorphism) strong monoidal functor $F: \text{Free}(S) \to \mathcal{C}$. Any diagram in the image of $F$ commutes.
