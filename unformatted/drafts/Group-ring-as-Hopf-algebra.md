...
A nice point of view on the group ring is that it is a kind of "linearised" version of a the group, where we have replaced the underlying set of the group $G$ with an underlying vector space (specifically, the *free* vector space on $G$). To see how we might go about this, let's revisit the definition of a group. 
