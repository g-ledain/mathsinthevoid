...
This is a nice proof, and has the virtue of being very hands-on. It is even more hands-on if you spell out the symmetry arguments and show that <span>$W$</span> is a direct complement by a direct argument rather than appealing to general facts about projection maps.

# Monoidal closure

The above proof is nice, but it doesn't feel especially revealing. In particular, the map <span>$r'$</span> is key to the whole thing but is rather pulled out of a hat. 
...
<div>$$(g.f)(v) = g.f(g^{-1}v) $$</div>
 ...
 - The invariants <span>$\textrm{Hom}_{V,W}^G$</span> are exactly the maps of <span>$G$</span>-representations


The first fact essentially tells us that the category <span>$G-\textrm{rep}$</span> of <span>$G$</span>-representations is a monoidal closed category, woth the tensor product as monoidal product. The second one has an important naturality property: any map of <span>$G$</span>-reps <span>$f: V\to W$</span> restricts to a map <span>$f: V^G \to W^G$</span> and 
...
The above proposition also holds when the short exact sequence is of <span>$G$</span>-representations - in that case the direct sum is then a direct sum *of* <span>$G$</span>-representations[^abelian-category].
...
<div>$$q \circ \pi^{\textrm{Hom}(W,V)}(s) = \pi^{\textrm{Hom}(W,W)}( q_*(s)) =  \pi^{\textrm{Hom}(W,W)}( q\circ s) = \pi^{\textrm{Hom}(W,W)}( \textrm{Id}_{W}) = \textrm{Id}_{W}$$</div>
...
<div>$$\pi^V: V^G \to V \textrm{ is the projection defined earlier}$$</div>
